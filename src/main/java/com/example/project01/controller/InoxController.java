...
public class InoxController {
        ...
        for(int i = 0; i < months; i++) {
            ...
            if(leavesList.size() > 0) {
                for(int l = 0; l < leavesList.size(); l++) {
                    ...
                    int leaveDate_int = Integer.parseInt(leaveDates);
                    int startDate_int = Integer.parseInt(annual.getStart_date());
                    if(leaveDates.contains(YearMonth)) {
                        if(leaveDate_int >= startDate_int) {
    ...
    public void sendSimpleMessage(String name , String type) throws UnirestException {
        String YOUR_DOMAIN_NAME = "sandbox5495de4cf3794a359a97712180c811a4.mailgun.org";
        String API_KEY = "c2bd7528d738e4955f75061f6daf0c26-73f745ed-33873ef6";
        HttpResponse<JsonNode> request = Unirest.post("https://api.mailgun.net/v3/" + YOUR_DOMAIN_NAME + "/messages")
                .basicAuth("api", API_KEY)
                .queryString("from", "INOXSOFT <admin@inoxsoft.com>")
                .queryString("to", "sj.choi@inoxsoft.com")
                .queryString("subject", "[휴가원] " + name + " " + type + " 신청하였습니다.")
                .queryString("text", "[휴가원] " + name + " " + type + " 신청하였습니다.")
                .asJson();
        if (request.getStatus() == 200) {
            System.out.println("Email sent successfully!");
        } else {
            System.err.println("Failed to send email. Status code: " + request.getStatus());
        }
    }
    ...
    public String leaveRequestSubmit(Model model,HttpServletRequest httpServletRequest,
                                     ...
                                     @RequestParam(value="reason",required=false) String reason) throws UnirestException {
        HttpSession session = httpServletRequest.getSession();
        Member member = (Member) session.getAttribute("user");
        ...
        sendSimpleMessage(member.getName() , typeKo);
        return "redirect:/inoxsoft/leave";
    }
    ...
    public String employeeDelete(Model model,
                                  @RequestParam(value="mno",required=false) int mno) throws ParseException {
        System.out.println(mno + "삭제 메서드");
        List<Leaves> leavesDTOList = leavesRepository.findLeavesByMno(mno);
        for(int i = 0; i < leavesDTOList.size(); i++) {
            int lno = leavesDTOList.get(i).getLno();
            leavesRepository.deleteByLno(lno);
        }
        List<Request> requestList = reqeustRepository.findByOneMnoList(mno);
        for(int i = 0; i < requestList.size(); i++) {
            int rno = requestList.get(i).getRno();
            reqeustRepository.deleteByRno(rno);
        }
        annualRepository.deleteByMno(mno);
